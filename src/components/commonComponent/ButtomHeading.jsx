...
const ButtomHeading = ({
  ...
  return (
    <div ref={textCon} className={`text-${color} w-full max-w-xl`}>
      <h1
        ref={headingRef}
        className="split text-3xl font-Montserrat font-bold md:text-5xl text-LightText"
      >
        {heading}
      </h1>
      <p
        ref={subheadingRef}
        className="split text-xl font-Poppins mt-5 text-LightText"
      >
        {paragraph1}
      </p>
      {paragraph2 && (
        <p
          ref={subheadin2gRef}
          className="split text-xl font-Poppins text-LightText text-shade-200 mt-3"
        >
          {paragraph2}
        </p>
      )}
